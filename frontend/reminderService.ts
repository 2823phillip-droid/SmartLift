...
export async function fireDueReminders(reminders: { id: number; text: string; scheduled_for: string }[]): Promise<void> {
  if (!Capacitor.isNativePlatform()) return;
  for (const r of reminders) {
    const LN = await getLocalNotifications();
    await LN.schedule({ notifications: [{ title: 'Askeo Reminder', body: r.text, schedule: false }] });
  }
}
