...
export default function ActiveWorkoutScreen({
  ...
  const buildPrescriptionHistory = (exercise: ExerciseEntry): SetRecord[] => {
    ...
    for (const s of lastSession) {
      history.push({
        set_index: s.set_index ?? undefined,
        actual_weight: toLbs(s.actual_weight),
        actual_reps: s.actual_reps,
        effort: s.effort,
    ...
    const load = async () => {
      ...
      for (const exercise of exercises) {
        try {
          ...
          const res = await withRetry(() => api.nextPrescription({
  ...
  useEffect(() => {
    ...
    const load = async () => {
      ...
      try {
        ...
        const initLast: Record<number, SetLog> = {};
        for (const s of setLogsData) {
          initCounts[s.exercise_entry_id] = (initCounts[s.exercise_entry_id] || 0) + 1;
          initLast[s.exercise_entry_id] = s;
        }
        loggedSetCountRef.current = initCounts;
  ...
  const handleEditSet = async (log: SetLog, field: "actual_weight" | "actual_reps" | "effort" | "form_quality", value: number | string) => {
  ...
  const handleDeleteSet = async (log: SetLog) => {
    ...
    if (remaining.length > 0) {
      const last = remaining.reduce((a, b) => (b.set_index > a.set_index ? b : a), remaining[0]);
      lastLoggedSetRef.current[log.exercise_entry_id] = last;
    } else {
      delete lastLoggedSetRef.current[log.exercise_entry_id];
    }
  };
  ...
  const logSet = async (): Promise<boolean> => {
    ...
    try {
      ...
      lastLoggedSetRef.current[currentExercise.id] = log;
  ...
  const nextSetDuringRest = (): { name: string; set: number; weight: number | string; reps: number | string } | null => {
    ...
    const existing = loggedSetCountRef.current[currentExercise.id] || 0;
    const nextSetIndex = existing + 1;

    if (nextSetIndex <= resolveDisplayTarget(currentExercise)) {
      // Prefer backend prescription weight when available — it already accounts
      // for the last session's progression. Fall back to local computation if
      // the backend hasn't loaded yet.
      const backendRx = prescriptions[currentExercise.id];
      let target: { weight: number; reps: number };
      if (backendRx?.next_weight != null && backendRx.next_weight > 0) {
        target = {
          weight: getUnitsPreference() === "imperial"
            ? Math.round(backendRx.next_weight)
            : Math.round(lbsToKg(backendRx.next_weight)),
          reps: backendRx.next_reps,
        };
      } else {
        target = getNextSetTarget(currentExercise);
      }
      return {
    ...
    if (nextExercise) {
      const backendRx = prescriptions[nextExercise.id];
      let target: { weight: number; reps: number };
      if (backendRx?.next_weight != null && backendRx.next_weight > 0) {
        target = {
          weight: getUnitsPreference() === "imperial"
        ...
        target = getNextSetTarget(nextExercise);
      }
      return {
            ...
            <div className="rounded-2xl border border-indigo-800/60 bg-indigo-950/30 p-4">
              ...
              <div className="text-xs text-slate-400 mt-1">
                {(() => {
                  ...
                  const w = Math.round(Number(n.weight) * 10) / 10;
