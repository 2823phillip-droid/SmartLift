        ...
        <div className="space-y-3">
          ...
          <div className="rounded-2xl border border-slate-800 bg-slate-900/50 p-4 space-y-3">
            <div className="flex items-center gap-2">
              ...
              <div>
                <div className="font-semibold text-sm">App Version</div>
                <div className="text-xs text-slate-500">Confirm this device is running the latest build.</div>
              </div>
            </div>
            <div className="flex items-center justify-between">
              <div>
                <div className="text-lg font-mono font-bold text-slate-200">{BUILD_INFO.short || "—"}</div>
                <div className="text-xs text-slate-500 mt-0.5">{BUILD_INFO.timestamp ? new Date(BUILD_INFO.timestamp).toLocaleString() : "—"}</div>
              </div>
              {BUILD_INFO.commit && (
                <button
