...
def coach_health(current_user: User = Depends(get_current_user_dep)):
    ...
    try:
        ...
        if resp.status_code == 200:
            return CoachHealthResponse(llm_available=True, model="tencent/hy4-preview", status="connected")
        elif resp.status_code == 429:
            return CoachHealthResponse(llm_available=True, model="tencent/hy4-preview", status="degraded")
        return CoachHealthResponse(llm_available=False, status="offline")
...
Tailor recommendations to their fitness profile, current phase, and training age."""
    ...
    for attempt in range(1):
        try:
            logger.info("[coach_chat] Nous request model=tencent/hy4-preview key_prefix=%s", api_key[:12] if api_key else "NONE")
            resp = httpx.post(
